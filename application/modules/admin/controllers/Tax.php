...
class tax extends CI_Controller{
	...
	function get_records(){
		...
		foreach ($query as $row){
			$actions = '<a class="btn btn-xs btn-info" href="'.base_url(MODULE.'/'.$this->class.'/form/'.$row->id_tax).'" title="Edit" data-rel="tooltip" data-placement="top">'.icon_action('edit').'</a>
						<a class="btn btn-xs btn-danger" onclick="DeleteConfirm(\''.base_url(MODULE.'/'.$this->class.'/delete').'\',\''.$row->id_tax.'\')" title="Delete" data-rel="tooltip" data-placement="top">'.icon_action('delete').'</a>';			
			$check_dfl = ($row->default == 1) ? 'checked' : '';
			$disabled = ($row->default == 1) ? 'disabled' : '';
			$default = '<label>
							<input class="ace ace-switch ace-switch-2" '.$check_dfl.' '.$disabled.' type="checkbox" onchange="ajaxcheck(\''.base_url(MODULE.'/'.$this->class.'/defaults').'\',\''.$row->id_tax.'#default'.'\',this)">
							<span class="lbl"></span>
						</label>';
	...
	function form($id=""){
		...
		if (!empty($id)){
			...
			$sql = $this->m_admin->get_table($this->table,'',array('id_tax'=>$id));			
			foreach ($sql as $row)
	...
	function proses(){
		$this->db->trans_start();	
		$id_tax = $this->input->post('id_tax',true);
		$code = $this->input->post('code',true);		
		$post['name']= $this->input->post('name',true);			
		$post['rate']= $this->input->post('rate',true);		
		$post['date_update']=$this->datenow;
		$post['update_by']=$this->addby;
		if (!empty($id_tax)){			
			$res = $this->m_admin->editdata($this->table,$post,array('id_tax'=>$id_tax));
	...
	function delete(){
		...
		if (empty($priv)){
			...
			$res = $this->m_admin->editdata($this->table,array('deleted'=>1),array('id_tax'=>$id));
			if ($res > 0){
	...
	function defaults(){
		...
		if (empty($priv)){
			...
			$res = $this->m_admin->editdata($this->table,array($obj=>$check),array('id_tax'=>$id));
			if ($res){
