...
class City extends CI_Controller{
	...
	function get_records(){
		...
		$total = count($this->m_admin->get_city());
		...
		$query = $this->m_admin->get_city('',$this->column());
		$this->m_admin->range_date($this->column(),$date_from,$date_to);
		$total = count($this->m_admin->get_city());
		...
		foreach ($query as $row){
			$actions = '<a class="btn btn-xs btn-info" href="'.base_url(MODULE.'/'.$this->class.'/form/'.$row->id_cities).'" title="Edit" data-rel="tooltip" data-placement="top">'.icon_action('edit').'</a>
						<a class="btn btn-xs btn-danger" onclick="DeleteConfirm(\''.base_url(MODULE.'/'.$this->class.'/delete').'\',\''.$row->id_cities.'\')" title="Delete" data-rel="tooltip" data-placement="top">'.icon_action('delete').'</a>';			
			$check_act = ($row->active == 1) ? 'checked' : '';
			$active = '<label>
							<input class="ace ace-switch ace-switch-2" '.$check_act.' type="checkbox" onchange="ajaxcheck(\''.base_url(MODULE.'/'.$this->class.'/active').'\',\''.$row->id_cities.'#active'.'\',this)">
							<span class="lbl"></span>
						</label>';
			$output['data'][] = array(
					$no,
	...
	function form($id=""){
		...
		if (!empty($id)){
			...
			$sql = $this->m_admin->get_city(array('A.id_cities'=>$id));			
			foreach ($sql as $row)
	...
	function proses(){
		$this->db->trans_start();	
		$id_cities = $this->input->post('id_cities',true);
		$code = $this->input->post('code',true);		
		$post['cities_name']= $this->input->post('name',true);	
		$post['id_province']= $this->input->post('province',true);
		$post['active']= (empty($this->input->post('active',true))) ? 0 : 1;
		$post['date_update']=$this->datenow;
		$post['update_by']=$this->addby;
		if (!empty($id_cities)){			
			$res = $this->m_admin->editdata($this->table,$post,array('id_cities'=>$id_cities));
	...
	function delete(){
		...
		if (empty($priv)){
			...
			$res = $this->m_admin->editdata($this->table,array('deleted'=>1),array('id_cities'=>$id));
			if ($res > 0){
	...
	function active(){
		...
		if (empty($priv)){
			$res = $this->m_admin->editdata($this->table,array($obj=>$check),array('id_cities'=>$id));
			if ($res){
