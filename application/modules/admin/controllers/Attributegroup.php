...
function get_records(){
		...
		$date_to = $_REQUEST['columns'][3]['search']['value'];
		...
		foreach ($query as $row){
			$actions = '<a class="btn btn-xs btn-info" href="'.base_url(MODULE.'/'.$this->class.'/form/'.$row->id_attribute_group).'" title="Edit" data-rel="tooltip" data-placement="top">'.icon_action('edit').'</a>
						<a class="btn btn-xs btn-danger" onclick="DeleteConfirm(\''.base_url(MODULE.'/'.$this->class.'/delete').'\',\''.$row->id_attribute_group.'\')" title="Delete" data-rel="tooltip" data-placement="top">'.icon_action('delete').'</a>';			
	...
	function form($id=""){
		...
		if (!empty($id)){
			...
			$sql = $this->m_admin->get_table($this->table,'*',array('id_attribute_group'=>$id));			
			foreach ($sql as $row)
	...
	function proses(){
		$this->db->trans_start();			
		$id_attribute_group = $this->input->post('id_attribute_group',true);
		$post['name_group']=$this->input->post('groupname',true);			
		$post['update_by']=$this->addby;		
		$post['date_update']=$this->datenow;		
		
		if (!empty($id_attribute_group)){
				$res = $this->m_admin->editdata($this->table,$post,array('id_attribute_group'=>$id_attribute_group));
				$alert = 'Edit attribute group successfull';
		}else{
				//addnew
				$post['date_add']=$this->datenow;
				$post['add_by']=$this->addby;
				$res = $this->m_admin->insertdata($this->table,$post);
				$alert = 'Add new attribute group  successfull';
		}
	...
	function delete(){
		...
		if (empty($priv)){
			...
			$res = $this->m_admin->editdata($this->table,array('deleted'=>1),array('id_attribute_group'=>$id));
			if ($res > 0){
				$msg = 'Delete attribute group successfull';
