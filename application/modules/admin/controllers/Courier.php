...
class Courier extends CI_Controller{
	...
	function get_records(){
		...
		foreach ($query as $row){
			$actions = '<a class="btn btn-xs btn-success" href="'.base_url(MODULE.'/'.$this->class.'/zone/'.$row->id_courier).'" title="Zone Coverage">'.icon_action('zone').'</a>
						<a class="btn btn-xs btn-info" href="'.base_url(MODULE.'/'.$this->class.'/form/'.$row->id_courier).'" title="Edit" data-rel="tooltip" data-placement="top">'.icon_action('edit').'</a>
	...
	function show_province($id_courier,$page){
		...
		if ($country_code == 'ID'){			
			...
			$res .='</select>';			
			$data['title1'] = 'District Zones Area';
			$data['title2'] = 'District Zones Coverage';
			$content = $this->load->view('courier/vw_district_area',$data,true);		
		}else{
			//jika selain indonesia				
			$data['title1'] = 'Country Zone Area';
			$data['title2'] = 'Country Zone Coverage';
			$data['table1'] = $this->class.'/table_zone_area_intern';
			$data['table2'] = $this->class.'/table_coverage_intern';
	...
	function show_zone(){
		...
		$data['p'] = $expl[2];
		$data['class'] = $this->class;		
		$tbl1 = $this->load->view($this->class.'/table_zone_area_local',$data,true);
		$tbl2 = $this->load->view($this->class.'/table_coverage_local',$data,true);
	...
	function zone_area_local($id_cities,$id_courier,$z){
		$output = array();
		//load datatable
		$this->m_admin->datatable();
	...
	function zone_area_intern($id_courier,$z){
		$output = array();
		//load datatable
		$this->m_admin->datatable();
		$where = '';
		...
		foreach ($query as $row){
			$output['data'][] = array(					
					...
					'<center><label><input type="checkbox" class="ace ace-checkbox-2 chkrow'.$z.'" name="chk'.$z.'[]" value="'.$row->id_country.'"><span class="lbl"></span></label></center>
					 <input type="hidden" name="data['.$row->id_country.'][country_code]" value="'.$row->country_code.'">',
					$row->id_country,
					$row->country_name,
	...
	function proses_zone(){		
		...
		$chk1 = $this->input->post('chkedit');//coverage area		
		...
		$data_new = $this->input->post('data');
		$data_cost = $this->input->post('datax');
			//jika row zone area dicek
			if (!empty($chk0)){
				//insert new covereage area
				/*
				 * jika pengiriman zone local
				 */
				...
				$priv = $this->m_admin->get_priv($this->access_code,'delete');
				if (empty($priv)){
					//delete coverage local area indonesia
					if (!empty($this->input->post('province')) && !empty($this->input->post('city'))){
						if ($this->input->post('edtbtn') == true){
							//edit cost local
							foreach ($chk1 as $val){
								$datax = $data_cost[$val];
								$post['id_currency'] = $_SESSION['id_currency'];
								...
								$wherecost = array('id_districts_to'=>$val,'id_courier'=>$id_courier,'id_country_to'=>$id_country);
								$res = $this->m_admin->editdata('tb_shipping_cost',$post_merge,$wherecost);
							}
							$notif  = ($res) ? 'Edit shipping cost zone coverage succesfull' : 'error';
						}else{
							//delete cost local
							foreach ($chk1 as $val){
								$where = array('id_districts'=>$val,'id_courier'=>$id_courier,'id_country'=>$id_country);							
							...
							$notif  = ($res) ? 'Delete Shipping cost zone coverage succesfull' : 'error';
						}						
					}else{
						if ($this->input->post('edtbtn') == true){
							//edit cost internationl
							foreach ($chk1 as $val){
								$datax = $data_cost[$val];
								$post['id_currency'] = $_SESSION['id_currency'];
								$post['date_update'] = $this->datenow;
								$post['update_by'] = $this->addby;
								$post_merge = array_merge($datax,$post);
								$wherecost = array('id_courier'=>$id_courier,'id_country_to'=>$val);
								$res = $this->m_admin->editdata('tb_shipping_cost',$post_merge,$wherecost);
							}
							$notif  = ($res) ? 'Edit shipping cost country zone succesfull' : 'error';
							
						}else{
							//delete cost international
							foreach ($chk1 as $val){
								$where = array('id_courier'=>$id_courier,'id_country'=>$val);
								$this->db->where($where);
								$res = $this->db->delete('tb_courier_zone');
							...
							$notif  = ($res) ? 'Delete Shipping cost country zone succesfull' : 'error';
						}
	...
	function zone_coverage_local($id_cities,$id_courier,$z){
		$output = array();
		//load datatable
		$this->m_admin->datatable();
		...
		foreach ($query as $row){
			$output['data'][] = array(
					$no,
					'<center><label><input type="checkbox" class="ace ace-checkbox-2 rowcost'.$z.'" name="chkedit[]" value="'.$row->id_districts.'"><span class="lbl"></span></label></center>',
					$row->id_districts,
					$row->districts_name,
					'<input type="text" name="datax['.$row->id_districts.'][weight_first]" class="input-mini costwght" placeholder="kg" id="cwf'.$no.'" value="'.$row->weight_first.'" onkeypress="return decimals(event,this.id)">',
					'<input type="text" name="datax['.$row->id_districts.'][price]" class="input-mini costprce" placeholder="'.$_SESSION['symbol'].'" id="cprc'.$no.'" value="'.$row->price.'" onkeypress="return decimals(event,this.id)">'
			);
			$no++;
		}
		echo json_encode($output);
	}
	function zone_coverage_intern($id_courier,$z){
		$output = array();
		//load datatable
		$this->m_admin->datatable();
		//tidak menampilkan area pengiriman local
		$where = array('B.id_courier'=>$id_courier,'B.id_districts_from < '=>1,'B.id_districts_to <'=>1);
		$total = count($this->m_admin->get_shipping_cost_country($where));
		$output['draw'] = $_REQUEST['draw'];
		$output['csrf_token'] = csrf_token()['hash'];
		$output['recordsTotal']= $output['recordsFiltered'] = $total;
		//date filter value already set index row column
		$this->m_admin->range_date($this->col_cost_country());
		$query = $this->m_admin->get_shipping_cost_country($where,$this->col_cost_country());
		$this->m_admin->range_date($this->col_cost_country());
		$total = count($this->m_admin->get_shipping_cost_country($where));
		$output['recordsFiltered'] = $total;
		$output['data'] = array();
		...
		foreach ($query as $row){
			$output['data'][] = array(
					$no,
					'<center><label><input type="checkbox" class="ace ace-checkbox-2 rowcost'.$z.'" name="chkedit[]" value="'.$row->id_country.'"><span class="lbl"></span></label></center>',
					$row->id_country,
					$row->country_name,
					'<input type="text" name="datax['.$row->id_country.'][weight_first]" class="input-mini costwght" placeholder="kg" id="cwf'.$no.'" value="'.$row->weight_first.'" onkeypress="return decimals(event,this.id)">',
					'<input type="text" name="datax['.$row->id_country.'][price]" class="input-mini costprce" placeholder="'.$_SESSION['symbol'].'" id="cprc'.$no.'" value="'.$row->price.'" onkeypress="return decimals(event,this.id)">'
			);
			$no++;
		}
		echo json_encode($output);
