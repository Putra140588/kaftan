...
class country extends CI_Controller{
	...
	function index(){
		...
		$body= (empty($priv)) ? $this->class.'/vw_country' : $priv['error'];
	...
	function get_records(){
		...
		$total = count($this->m_admin->get_country());
		...
		$query = $this->m_admin->get_country('',$this->column());
		$this->m_admin->range_date($this->column(),$date_from,$date_to);
		$total = count($this->m_admin->get_country());
		...
		foreach ($query as $row){
			$actions = '<a class="btn btn-xs btn-info" href="'.base_url(MODULE.'/'.$this->class.'/form/'.$row->id_country).'" title="Edit" data-rel="tooltip" data-placement="top">'.icon_action('edit').'</a>
						<a class="btn btn-xs btn-danger" onclick="DeleteConfirm(\''.base_url(MODULE.'/'.$this->class.'/delete').'\',\''.$row->id_country.'\')" title="Delete" data-rel="tooltip" data-placement="top">'.icon_action('delete').'</a>';			
	...
	function form($id=""){
		...
		if (!empty($id)){
			...
			$sql = $this->m_admin->get_table($this->table,'',array('id_country'=>$id));			
			foreach ($sql as $row)
	...
	function proses(){
		...
		$post['country_name']= $this->input->post('name',true);	
		$post['country_code']= $this->input->post('code',true);
		$post['id_currency']= $this->input->post('currency',true);
		$flag = $_FILES['flag']['name'];		
		if ($flag != '')
		{
			//proses upload gambar
			$setting['upload_path'] = './assets/images/flag/';
			$setting['file_name'] = $flag;
			$setting['var_name'] = 'flag';
			$this->m_admin->upload_image($setting);
			$post['flag'] = $flag;
		}
		if (!empty($id_country)){			
			$res = $this->m_admin->editdata($this->table,$post,array('id_country'=>$id_country));
	...
	function delete(){
		...
		if (empty($priv)){
			...
			$res = $this->m_admin->editdata($this->table,array('deleted'=>1),array('id_country'=>$id));
			if ($res > 0){
	...
	function active(){
		...
		if (empty($priv)){
			$res = $this->m_admin->editdata($this->table,array($obj=>$check),array('id_country'=>$id));
			if ($res){
