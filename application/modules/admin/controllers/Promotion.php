...
class Promotion extends CI_Controller{
	...
	function get_records(){
		...
		$total = count($this->m_admin->get_promotion());
		...
		$query = $this->m_admin->get_promotion('',$this->column());
		$this->m_admin->range_date($this->column(),$date_from,$date_to);
		$total = count($this->m_admin->get_promotion());
		...
		foreach ($query as $row){
			$actions = '<a class="btn btn-xs btn-info" href="'.base_url(MODULE.'/'.$this->class.'/form/'.$row->id_promotion).'" title="Edit" data-rel="tooltip" data-placement="top">'.icon_action('edit').'</a>
						<a class="btn btn-xs btn-danger" onclick="DeleteConfirm(\''.base_url(MODULE.'/'.$this->class.'/delete').'\',\''.$row->id_promotion.'\')" title="Delete" data-rel="tooltip" data-placement="top">'.icon_action('delete').'</a>';			
			$check_act = ($row->active == 1) ? 'checked' : '';
			$active = '<label>
							<input class="ace ace-switch ace-switch-2" '.$check_act.' type="checkbox" onchange="ajaxcheck(\''.base_url(MODULE.'/'.$this->class.'/active').'\',\''.$row->id_promotion.'#active'.'\',this)">
							<span class="lbl"></span>
						</label>';
			$output['data'][] = array(
					$no,
	...
	function form($id=""){
		...
		if (!empty($id)){
			...
			$sql = $this->m_admin->get_table($this->table,'',array('id_promotion'=>$id));			
			foreach ($sql as $row)
	...
	function proses(){
		...
		$post['title_promo']= $this->input->post('title',true);			
		...
		if (!empty($id_promotion)){			
			$res = $this->m_admin->editdata($this->table,$post,array('id_promotion'=>$id_promotion));
	...
	function delete(){
		...
		if (empty($priv)){
			...
			$res = $this->m_admin->editdata($this->table,array('deleted'=>1),array('id_promotion'=>$id));
			if ($res > 0){
	...
	function active(){
		...
		if (empty($priv)){
			$res = $this->m_admin->editdata($this->table,array($obj=>$check),array('id_promotion'=>$id));
			if ($res){
