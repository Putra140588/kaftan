...
class Warehouse extends CI_Controller{
	...
	function index(){
		...
		$body= (empty($priv)) ? $this->class.'/vw_warehouse' : $priv['error'];
	...
	function get_records(){
		...
		$total = count($this->m_admin->get_warehouse());
		...
		$query = $this->m_admin->get_warehouse('',$this->column());
		$this->m_admin->range_date($this->column(),$date_from,$date_to);
		$total = count($this->m_admin->get_warehouse());
		...
		foreach ($query as $row){
			$actions = '<a class="btn btn-xs btn-info" href="'.base_url(MODULE.'/'.$this->class.'/form/'.$row->id_warehouse).'" title="Edit" data-rel="tooltip" data-placement="top">'.icon_action('edit').'</a>
						<a class="btn btn-xs btn-danger" onclick="DeleteConfirm(\''.base_url(MODULE.'/'.$this->class.'/delete').'\',\''.$row->id_warehouse.'\')" title="Delete" data-rel="tooltip" data-placement="top">'.icon_action('delete').'</a>';			
	...
	function form($id=""){
		...
		if (!empty($id)){
			...
			$sql = $this->m_admin->get_warehouse(array('id_warehouse'=>$id));			
			foreach ($sql as $row)
	...
	function show_city($id_cities=''){
		...
		$res .=$this->m_content->chosen_city($id,$id_cities,$this->class,'form-ajax');
	...
	function show_districts($id_districts=''){
		$res='';
		$id = $this->input->post('value',true);
		$res .=$this->m_content->chosen_districts($id,$id_districts,'form-ajax');
		$res .=$this->m_content->load_choosen();
		echo json_encode(array('csrf_token'=>csrf_token()['hash'],'error'=>0,'element'=>$res));
	}
	function delete(){
