...
class Employee extends CI_Controller{
	...
	function form($id=""){
		...
		$data['notif']= (empty($priv)) ? '' : $priv['notif'];
		if ($this->session->userdata('code_dep') != 'SA'){
			//jika login bukan sebagai superadmin maka departement superadmin tidak ditampilkan
			$this->db->where('code_departement <>','SA');
		}
		$data['department'] = $this->m_admin->get_table('tb_departement','*',array('deleted'=>0));
	...
	function proses(){
		...
		$post['employee_code']=$this->input->post('empcode',true);
		$post['id_departement']=$this->input->post('department',true);
