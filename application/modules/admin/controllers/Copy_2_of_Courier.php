...
class Courier extends CI_Controller{
	...
	function show_province($id_courier,$page){
		...
		if ($country_code == 'ID'){			
			...
			$res .='</select>';
			if ($page == 'zone'){
				$title1 = 'District Zones Area';
				$title2 = 'District Zones Coverage';
			}else{
				$title1 = '';
				$title2 = 'District Zones Shipping Cost';
			}
			$data['title1'] = $title1;
			$data['title2'] = $title2;
			...
			$data['table1'] = $this->class.'/table_country_area';
			$data['table2'] = $this->class.'/table_country_coverage_cost';
	...
	function get_shipp_cost_country($id_courier,$z){
		$output = array();
		//load datatable
		$this->m_admin->datatable();
