...
class Group extends CI_Controller{
	...
	function get_records(){
		...
		foreach ($query as $row){
			$actions = '<a class="btn btn-xs btn-info" href="'.base_url(MODULE.'/'.$this->class.'/form/'.$row->id_group).'" title="Edit" data-rel="tooltip" data-placement="top">'.icon_action('edit').'</a>
			...
			$default = '<label>
							<input class="ace ace-switch ace-switch-2" '.$check_dfl.' '.$disabled.' type="checkbox" onchange="ajaxcheck(\''.base_url(MODULE.'/'.$this->class.'/defaults').'\',\''.$row->id_group.'#default'.'\',this)">
							<span class="lbl"></span>
						</label>';
	...
	function form($id=""){
		...
		if (!empty($id)){
			...
			$sql = $this->m_admin->get_table($this->table,'*',array('id_group'=>$id));			
			foreach ($sql as $row)
	...
	function defaults(){
		...
		if (empty($priv)){
			...
			$res = $this->m_admin->editdata($this->table,array($obj=>$check),array('id_group'=>$id));
			if ($res){
